...
// time - about 1 second (was about a minute before priority queue)
...
function BFS(root, goal, d)
{
    parent = {}
    val = {}
    queue = new FastPriorityQueue((a,b) => a[1] < b[1]);
    queue.add([root,0])
    ...
    while(queue.size > 0)
    {
        v = queue.poll()
        parsed = v[0]
        adj = [parsed-1, parsed+1, parsed-YS, parsed+YS]
        adj.forEach(e => {
            alt = val[v] + d[e]
            if(d[e] != undefined && (parent[e] == undefined || alt < val[e]))
            {
                parent[e] = parsed 
                val[e] = val[parsed] + d[e]
                queue.add([e, val[e]])
            }
        })
    }
